...
/**
 * Day 13 & 15: the stage of the task lifecycle a [TaskState] is currently in.
 *
 ...
 * - [DONE]: terminal — the task is finished successfully after passing validation.
 * - [CANCELLED]: terminal — the task was cancelled before completion.
 *
 * See [TaskTransitionTable] for the allowed transition table between these stages.
 */
enum class TaskStage { PLANNING, EXECUTION, VALIDATION, DONE, CANCELLED }

/**
 * Day 15: recorded outcome of validating task execution results.
 * Required before transitioning from [TaskStage.VALIDATION] to [TaskStage.DONE].
 */
enum class ValidationOutcome { NOT_RUN, PASSED, FAILED }
...
/**
 * Day 13 & 15: the formalized state of a single task the agent and user are working through
 ...
 * @property validationOutcome Day 15: outcome of the validation check in [TaskStage.VALIDATION].
 * @property validationNote Day 15: optional explanation or findings from validation.
 * @property planApproved Day 15: whether the plan was formally approved to enter [TaskStage.EXECUTION].
 * @property stageEnteredAt Day 15: timestamp when the current [stage] was entered.
 * @property cancellationReason Day 15: reason why the task was cancelled, if [stage] is [TaskStage.CANCELLED].
 */
@Serializable
data class TaskState(
    ...
    val validationOutcome: ValidationOutcome = ValidationOutcome.NOT_RUN,
    ...
    /** `true` if the task is in a terminal stage ([TaskStage.DONE] or [TaskStage.CANCELLED]). */
    val isTerminal: Boolean get() = stage == TaskStage.DONE || stage == TaskStage.CANCELLED
